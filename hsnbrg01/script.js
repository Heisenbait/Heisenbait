document.addEventListener('DOMContentLoaded', () => {
  ...
  const header = $('.header');
  const infoMSC = $('#info-MSC');
  const songNameEl = $('#songName');
  const songImageEl = $('#songImage');
  ...
  const albumMetaEl = $('#albumMeta');

  const views = {
    inicio: { el: $('#inicio'), btn: $('#inicioBtn') },
    msc: { el: infoMSC, btn: $('#mscBtn') },
    icfrd: { el: $('#info-ICFRD'), btn: $('#icfrdBtn') }
  };
  ...
  function showView(name) {
    ...
    header.style.display = name === 'inicio' ? 'block' : 'none';
    searchBtn.disabled = name !== 'msc';
    window.scrollTo(0, 0);
    if (name === 'msc') initPlayer();
  }
  ...
  /* ---------- Lista de canciones ---------- */
  function renderTracks(indices = songs.map((_, i) => i)) {
    if (indices.length === 0) {
      trackListEl.innerHTML = '<div class="no-results">No se encontraron canciones</div>';
      return;
    }
    trackListEl.innerHTML = indices.map((i, pos) => {
      const s = songs[i];
      const active = i === currentSongIndex ? ' active' : '';
      return `
        <div class="track${active}" data-index="${i}" role="button" tabindex="0">
          <span class="number">${pos + 1}</span>
          <div class="title-container">
            <div class="title">${s.name}</div>
            <div class="song-text">${s.text}</div>
          </div>
          <span class="time">${s.duration || '0:00'}</span>
        </div>`;
    }).join('');
  }
  ...
  function highlightActive() {
    ...
    const active = $(`.track[data-index="${currentSongIndex}"]`);
  ...
  function updateAlbumMeta() {
    const totalSeconds = songs.reduce((sum, s) => {
      const [m, sec] = (s.duration || '0:00').split(':').map(Number);
      return sum + m * 60 + sec;
    }, 0);
    const h = Math.floor(totalSeconds / 3600);
    const min = Math.floor((totalSeconds % 3600) / 60);
    albumMetaEl.textContent = `${songs.length} canciones · ${h} h ${min} min`;
  }

  /* ---------- Búsqueda ---------- */
  function openSearch() {
    if (infoMSC.style.display !== 'block') return;
    navbar.classList.add('search-active');
    searchInput.focus();
  }

  function closeSearch() {
    if (!navbar.classList.contains('search-active')) return;
    navbar.classList.remove('search-active');
    searchInput.value = '';
    if (playerReady) renderTracks();
  }
  ...
  searchInput.addEventListener('input', () => {
    ...
    if (!term) return renderTracks();
    const matches = [];
    songs.forEach((s, i) => {
      if (s.name.toLowerCase().includes(term) || s.text.toLowerCase().includes(term)) matches.push(i);
    });
    renderTracks(matches);
  });
  ...
  function tintBackground(img) {
    if (typeof ColorThief === 'undefined') return;
    try {
      const [r, g, b] = new ColorThief().getColor(img);
      const d = (v) => Math.floor(v * 0.7);
      infoMSC.style.setProperty('--tint', `rgb(${d(r)}, ${d(g)}, ${d(b)})`);
    } catch (err) {
...
function playSong(index, addToHistory = true) {
    ...
    const start = (loaded) => {
      ...
      audio.src = song.src;
      highlightActive();
      if (loaded) tintBackground(img);
      audio.play().catch((err) => console.error('Error al reproducir:', err));
    };

    img.onload = () => start(true);
    img.onerror = () => start(false); // si falta la portada, igual suena
    img.src = song.image;
  }

  function togglePlayPause() {
    if (!audio.src) return playSong(isShuffleOn ? nextShuffleIndex() : 0);
  ...
  function toggleShuffle() {
    ...
    randomBtn.setAttribute('aria-pressed', isShuffleOn);
    if (isShuffleOn) {
      generateShuffleQueue();
      playSong(nextShuffleIndex());
    }
  }

  function nextSong() {
    playSong(isShuffleOn ? nextShuffleIndex() : (currentSongIndex + 1) % songs.length);
  }
  ...
  // El botón siempre refleja el estado real del audio
  ...
  /* ---------- Barra de progreso (mouse y touch) ---------- */
  function seek(e) {
    const rect = progressBar.getBoundingClientRect();
    const ratio = Math.max(0, Math.min((e.clientX - rect.left) / rect.width, 1));
    if (audio.duration && !isNaN(audio.duration)) audio.currentTime = ratio * audio.duration;
  }
  ...
  const endDrag = () => { dragging = false; progressBar.classList.remove('dragging'); };
  progressBar.addEventListener('pointerup', endDrag);
  progressBar.addEventListener('pointercancel', endDrag);

  /* ---------- Efecto 3D de la portada (solo con mouse) ---------- */
  if (window.matchMedia('(hover: hover)').matches) {
    songImageEl.addEventListener('mousemove', (e) => {
      const { offsetX, offsetY } = e;
      const { offsetWidth, offsetHeight } = songImageEl;
      const rotateX = ((offsetY / offsetHeight) - 0.5) * 10;
      const rotateY = ((offsetX / offsetWidth) - 0.5) * 10;
      songImageEl.style.transform = `perspective(500px) rotateX(${-rotateX}deg) rotateY(${rotateY}deg) scale(1.05)`;
    });
    songImageEl.addEventListener('mouseleave', () => {
      songImageEl.style.transform = 'perspective(500px) rotateX(0) rotateY(0) scale(1)';
    });
  }

  // Los botones ◀◀ ▶▶ del HTML usan onclick="prevSong()" / "nextSong()"
